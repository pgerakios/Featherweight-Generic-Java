...
let reservedWords = [
  (* Keywords *)
  ("Object", fun i -> Parser.OBJECT i);
  ("class", fun i -> Parser.CLASS i);
  ("this", fun i -> Parser.THIS i);
  ("return", fun i -> Parser.RETURN i);
  ("new", fun i -> Parser.NEW i);
  ("extends", fun i -> Parser.EXTENDS i);
  ("super", fun i -> Parser.SUPER i);

(*  ("Top", fun i -> Parser.TTOP i);
  ("Top", fun i -> Parser.TTOP i);
  ("Bot", fun i -> Parser.TBOT i);
  ("Bool", fun i -> Parser.TBOOL i);
  ("True", fun i -> Parser.TTRUE i);
  ("False", fun i -> Parser.TFALSE i);
  ("Base", fun i -> Parser.TBASE i);
  ("Obj", fun i -> Parser.TRECORD i);
  ("Lang", fun i -> Parser.TLANG i);
  ("Label", fun i -> Parser.TLABEL i);
...
let createID i str =
  ...
  with _ ->
    ...
    if first >= 'A' &&  first <= 'Z' then
       ...
       Parser.BCID {i=i;v=String.sub str 1 (String.length str - 1)}
    else if first = '$' then
       Parser.TCID {i=i;v=String.sub str 1 (String.length str - 1)}
    else 
